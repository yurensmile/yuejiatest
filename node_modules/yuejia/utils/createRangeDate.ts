import { Data } from '../component/Picker/Item';
import { Props as DatePickerProps } from '../component/DatePicker';
...
function getDateData(minDate: Date, maxDate: Date):  Data[] {
  ...
  // console.log( 'yearData', getYearData(minDate, maxDate))
  // 为了解决闰年也是28天的问题
  ...
  for (let i = 0; i < yearmonth.length; i++) {
    ...
    if (month && month.length) {
      for (let l = 0; l < month.length; l++) {
        ...
        // console.log(minDate.getFullYear(), yearmonth[i].value, minDate.getMonth() + 1, month[l].value)
